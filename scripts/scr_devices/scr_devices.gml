function scr_devices_laserPointer(level) {
...
function scr_devices_watch(level) {
	...
	if (type == "analog") {
		...
		var list = [low, low + 0.05, low + 0.05, low + 0.05, low + 0.05];
		var len = array_length(list);
		var index = irandom_range_biased(0, len - 1, LOOT_BIAS, true);
		var num = list[index];
	...
	if (regenType == "oximetric") {
		...
		var regen = irandom_range_biased(low, high, LOOT_BIAS, true);
